...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    //   dist(double dX, double y1, double x2, double y2)
    ...
    for(unsigned int j = 0; j < mappedObservations.size(); j++) {
      ...
      double weight = ( 1/(2*M_PI*stdLandmarkX*stdLandmarkY)) * exp( -( dX*dX/(2*stdLandmarkY*stdLandmarkX) + (dY*dY/(2*stdLandmarkY*stdLandmarkY)) ) );
      if (weight == 0) {
